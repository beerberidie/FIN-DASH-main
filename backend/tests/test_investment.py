...
def test_update_price(investment_id):
    ...
    price_update = {"current_price": 180.25, "last_updated": str(date.today())}

    response = requests.patch(
        f"{BASE_URL}/investments/{investment_id}/price", json=price_update
    )
    assert response.status_code == 200, f"Failed: {response.text}"
...
def test_create_buy_transaction(investment_id):
    ...
    response = requests.post(
        f"{BASE_URL}/investments/transactions", json=transaction_data
    )
    assert response.status_code == 201, f"Failed: {response.text}"
    ...
    print(
        f"✓ Created buy transaction: {transaction['quantity']} shares @ ${transaction['price']}"
    )
...
def test_investment_updated_after_buy(investment_id):
    ...
    investment = response.json()
    assert (
        investment["quantity"] == 10
    ), f"Expected quantity 10, got {investment['quantity']}"
    assert (
        investment["average_cost"] == 175.50
    ), f"Expected avg cost 175.50, got {investment['average_cost']}"
    print(
        f"✓ Investment updated: {investment['quantity']} shares @ avg ${investment['average_cost']}"
    )
...
def test_create_another_buy_transaction(investment_id):
    ...
    transaction = response.json()
    print(
        f"✓ Created second buy transaction: {transaction['quantity']} shares @ ${transaction['price']}"
    )
...
def test_average_cost_calculation(investment_id):
    ...
    assert (
        investment["quantity"] == expected_quantity
    ), f"Expected quantity {expected_quantity}, got {investment['quantity']}"
    assert (
        abs(investment["average_cost"] - expected_avg_cost) < 0.01
    ), f"Expected avg cost {expected_avg_cost}, got {investment['average_cost']}"
    print(
        f"✓ Average cost calculated correctly: {investment['quantity']} shares @ avg ${investment['average_cost']}"
    )
...
def test_create_sell_transaction(investment_id):
    ...
    print(
        f"✓ Created sell transaction: {transaction['quantity']} shares @ ${transaction['price']}"
    )
...
def test_list_transactions(investment_id):
    ...
    response = requests.get(
        f"{BASE_URL}/investments/transactions/list?investment_id={investment_id}"
    )
    assert response.status_code == 200, f"Failed: {response.text}"
...
def test_get_investment_performance(investment_id):
    ...
    print(
        f"  - Profit/Loss: ${performance['profit_loss']:.2f} ({performance['profit_loss_percentage']:.2f}%)"
    )
...
def test_get_portfolio_summary():
    ...
    print(
        f"  - Profit/Loss: ${summary['total_profit_loss']:.2f} ({summary['total_profit_loss_percentage']:.2f}%)"
    )
    print(f"  - Asset Allocation:")
    for inv_type, data in summary["by_type"].items():
        print(
            f"    * {inv_type}: {data['percentage']:.1f}% (${data['total_value']:.2f})"
        )
