...
def test_debt_endpoints():
    ...
    for debt in debts:
        print(
            f"  - {debt['name']}: R{debt['current_balance']:.2f} @ {debt['interest_rate']}%"
        )
    print()
    ...
    payment_data = {"amount": 1000, "notes": "Monthly payment"}
    response = requests.post(
        f"{BASE_URL}/debts/{credit_card_id}/payment", json=payment_data
    )
    print(f"Status: {response.status_code}")
    ...
    plan_data = {"extra_payment": 2000, "strategy": "avalanche"}
    ...
    plan_data = {"extra_payment": 2000, "strategy": "snowball"}
    response = requests.post(f"{BASE_URL}/debts/payoff-plan", json=plan_data)
    ...
    plan_data = {"extra_payment": 2000, "strategy": "both"}
    response = requests.post(f"{BASE_URL}/debts/payoff-plan", json=plan_data)
    comparison = response.json()
    print(
        f"Avalanche: {comparison['avalanche']['total_months']} months, R{comparison['avalanche']['total_interest']:.2f} interest"
    )
    print(
        f"Snowball: {comparison['snowball']['total_months']} months, R{comparison['snowball']['total_interest']:.2f} interest"
    )
...
def test_report_endpoints():
    ...
    for cat in report["top_categories"][:3]:
        print(
            f"  - {cat['category_name']}: R{cat['amount']:.2f} ({cat['percentage']:.1f}%)"
        )
    print(f"\nInsights:")
