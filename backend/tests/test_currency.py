...
def test_get_currency():
    ...
    print(
        f"✓ Retrieved currency: {currency['code']} - {currency['name']} ({currency['symbol']})"
    )
...
def test_list_exchange_rates():
    ...
    response = requests.get(
        f"{BASE_URL}/currencies/exchange-rates/list?from_currency=USD"
    )
    assert response.status_code == 200, f"Failed: {response.text}"
...
def test_currency_conversion():
    ...
    conversion_data = {"amount": 100.0, "from_currency": "USD", "to_currency": "ZAR"}
    ...
    print(
        f"✓ Converted {result['original_amount']} {result['from_currency']} = "
...
def test_same_currency_conversion():
    ...
    conversion_data = {"amount": 100.0, "from_currency": "ZAR", "to_currency": "ZAR"}
    ...
    result = response.json()
    assert (
        result["converted_amount"] == 100.0
    ), "Same currency should return same amount"
    assert result["exchange_rate"] == 1.0, "Same currency rate should be 1.0"
    print(
        f"✓ Same currency conversion: {result['original_amount']} ZAR = {result['converted_amount']} ZAR"
    )
...
def test_multi_currency_transaction():
    ...
    print(
        f"✓ Created transaction: {transaction['amount']} {transaction['currency']} - {transaction['description']}"
    )
...
def test_summary_with_multi_currency():
    ...
    print(
        f"✓ Monthly expenses: {summary['monthly_expenses']} {summary['base_currency']}"
    )
