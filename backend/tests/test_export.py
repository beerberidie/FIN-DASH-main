...
def test_export_transactions_pdf():
    ...
    # Verify file exists
    assert _check_file_exists(
        result["file_path"]
    ), f"File not created: {result['file_path']}"
    assert result["file_size"] > 0, "File is empty"

    print(
        f"✓ Exported transactions to PDF: {result['filename']} ({result['file_size']} bytes)"
    )
...
def test_export_transactions_excel():
    ...
    print(
        f"✓ Exported transactions to Excel: {result['filename']} ({result['file_size']} bytes)"
    )
...
def test_export_transactions_csv():
    ...
    print(
        f"✓ Exported transactions to CSV: {result['filename']} ({result['file_size']} bytes)"
    )
...
def test_export_financial_summary_pdf():
    ...
    print(
        f"✓ Exported financial summary to PDF: {result['filename']} ({result['file_size']} bytes)"
    )
...
def test_export_investment_portfolio_pdf():
    ...
    print(
        f"✓ Exported investment portfolio to PDF: {result['filename']} ({result['file_size']} bytes)"
    )
...
def test_export_investment_portfolio_excel():
    ...
    print(
        f"✓ Exported investment portfolio to Excel: {result['filename']} ({result['file_size']} bytes)"
    )
...
def test_export_debt_report_pdf():
    ...
    print(
        f"✓ Exported debt report to PDF: {result['filename']} ({result['file_size']} bytes)"
    )
...
def test_list_exports():
    ...
    if exports:
        print(
            f"  Latest export: {exports[0]['filename']} ({exports[0]['export_type']})"
        )
...
def test_export_with_type_filter():
    ...
    response = requests.post(
        f"{BASE_URL}/export/transactions/pdf", params={"transaction_type": "income"}
    )
    assert response.status_code == 200, f"Failed: {response.text}"
    ...
    response = requests.post(
        f"{BASE_URL}/export/transactions/excel", params={"transaction_type": "expense"}
    )
    assert response.status_code == 200, f"Failed: {response.text}"
