...
def test_budgets():
    ...
    if response.status_code == 200:
        ...
        update_data = {"needs_planned": 11000.00, "notes": "Updated test budget"}
        print(f"\n6. PUT /api/budgets/{created_budget_id}")
        print(f"Payload: {json.dumps(update_data, indent=2)}")
        response = requests.put(
            f"{BASE_URL}/budgets/{created_budget_id}", json=update_data
        )
...
def test_goals():
    ...
    if response.status_code == 200:
        ...
        response = requests.post(
            f"{BASE_URL}/goals/{created_goal_id}/contribute", json=contribution
        )
        ...
        # Test 7: Update goal
        update_data = {"name": "Updated Test Vacation Fund", "target_amount": 20000.00}
