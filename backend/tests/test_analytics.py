...
def test_income_trend_analysis():
    ...
    print(f"✓ Income trend: {result['trend_direction']} ({result['trend_percentage']:.1f}%)")
...
def test_expense_trend_analysis():
    ...
    print(f"✓ Expense trend: {result['trend_direction']} ({result['trend_percentage']:.1f}%)")
...
def test_yoy_comparison():
    ...
    print(f"  Income: {income_comp['current_value']:.2f} vs {income_comp['previous_value']:.2f} ({income_comp['change_percentage']:.1f}%)")
    print(f"  Expenses: {expense_comp['current_value']:.2f} vs {expense_comp['previous_value']:.2f} ({expense_comp['change_percentage']:.1f}%)")
    print(f"  Savings rate: {result['savings_rate_current']:.1f}% vs {result['savings_rate_previous']:.1f}%")
...
def test_spending_patterns():
    ...
    if patterns:
        for i, pattern in enumerate(patterns[:3], 1):
            print(f"  {i}. {pattern['category']}: {pattern['pattern_type']} - {pattern['description']}")
...
def test_income_predictions():
    ...
    response = requests.get(f"{BASE_URL}/analytics/predictions/income?periods_ahead=3&method=moving_average")
    assert response.status_code == 200, f"Failed: {response.text}"
    ...
    for pred in result['predictions']:
        print(f"  {pred['period']}: {pred['predicted_value']:.2f} (CI: {pred['confidence_interval_low']:.2f} - {pred['confidence_interval_high']:.2f})")
...
def test_expense_predictions():
    ...
    response = requests.get(f"{BASE_URL}/analytics/predictions/expenses?periods_ahead=3&method=linear_regression")
    assert response.status_code == 200, f"Failed: {response.text}"
...
def test_seasonal_predictions():
    ...
    response = requests.get(f"{BASE_URL}/analytics/predictions/expenses?periods_ahead=6&method=seasonal")
    assert response.status_code == 200, f"Failed: {response.text}"
...
def test_financial_health_score():
    ...
    print(f"✓ Financial Health Score: {result['overall_score']:.1f}/100 ({result['overall_status']})")
    print(f"\n  Individual Metrics:")
