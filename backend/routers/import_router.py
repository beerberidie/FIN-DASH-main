...
async def preview_categorization(
    ...
    try:
        ...
        if not column_mapping:
            raise HTTPException(status_code=400, detail=f"Unknown bank format: {bank_format}")
        ...
        for row in rows[:10]:
            try:
                # Validate row
                is_valid, error = import_service.validate_transaction(row, column_mapping)
                if not is_valid:
                    continue
                ...
                previews.append(CategorizationPreview(
