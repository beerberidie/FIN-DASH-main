...
class PayoffPlanRequest(BaseModel):
    ...
    strategy: Optional[str] = Field(
        default="both", pattern="^(avalanche|snowball|both)$"
    )
...
async def record_payment(debt_id: str, payment: DebtPayment):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to record payment: {str(e)}"
        )
...
async def get_payoff_plan(request: PayoffPlanRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to calculate payoff plan: {str(e)}"
        )
...
async def get_debt_summary():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get debt summary: {str(e)}"
        )
