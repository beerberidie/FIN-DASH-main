...
class HealthService:
    ...
    def calculate_health_score(
        ...
        savings_metric = self._calculate_savings_rate_metric(reference_date, base_currency)
        ...
        budget_metric = self._calculate_budget_adherence_metric(reference_date, base_currency)
        ...
        total_score = sum(m.score * self._get_metric_weight(m.name) / 100 for m in metrics)
    ...
    def _calculate_emergency_fund_metric(self, base_currency: str) -> HealthMetric:
        ...
        if months >= 6:
            ...
            score = 50
            status = "fair"
            recommendation = "Increase your emergency fund to at least 3 months of expenses."
        else:
            score = 25
            status = "poor"
    ...
    def _calculate_debt_metric(
        ...
        if ratio <= 0:
            ...
            score = 50
            status = "fair"
            recommendation = "Your debt-to-income ratio is high. Focus on debt reduction strategies."
        else:
            score = 25
            status = "poor"
    ...
    def _calculate_budget_adherence_metric(
        ...
        if adherence >= 95:
            ...
            score = 85
            status = "good"
            recommendation = "Good budget discipline. Fine-tune your budget for even better results."
        elif adherence >= 70:
            score = 70
            status = "fair"
            recommendation = "Review your budget regularly and track spending more closely."
        else:
            score = 50
            status = "poor"
            recommendation = "Improve budget tracking and adjust budgets to be more realistic."
    ...
    def _calculate_investment_metric(self) -> HealthMetric:
        ...
        if diversification >= 80:
            ...
            score = 60
            status = "fair"
            recommendation = "Improve diversification by investing in different asset classes."
        else:
            score = 40
            status = "poor"
    ...
    def _get_debt_to_income_ratio(self, reference_date: date_type, base_currency: str) -> float:
        """Calculate debt-to-income ratio."""
        ...
        try:
            ...
            total_monthly_payment = sum(debt.minimum_payment for debt in debts if not debt.is_paid_off)
        except:
    ...
    def _get_budget_adherence(self, reference_date: date_type, base_currency: str) -> float:
        ...
        for txn in transactions:
            ...
            category_spending[category_id] = category_spending.get(category_id, 0) + amount
    ...
    def _get_networth_trend(self, base_currency: str) -> str:
        ...
        changes = [networth_values[i+1] - networth_values[i] for i in range(len(networth_values)-1)]
