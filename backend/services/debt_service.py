...
class DebtService:
    ...
    def _calculate_payoff_schedule(self, debts: List[Dict], extra_payment: float, method: str) -> Dict:
        """
        ...
        while any(d['balance'] > 0 for d in working_debts):
            ...
            # Distribute payments
            available_payment = sum(d['minimum_payment'] for d in working_debts if d['balance'] > 0) + extra_payment
        ...
        for debt in debts:
            ...
            debt_summary.append({
                ...
                'payoff_date': (datetime.now() + timedelta(days=30 * payoff_month)).strftime('%Y-%m-%d') if payoff_month > 0 else None
            })
        ...
        return {
            ...
            'payoff_date': (datetime.now() + timedelta(days=30 * month)).strftime('%Y-%m-%d') if month > 0 else None,
            'debts': debt_summary,
