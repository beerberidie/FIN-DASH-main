...
class CurrencyService:
        ...
        if not currencies:
            ...
            for curr_data in DEFAULT_CURRENCIES:
                ...
                csv_manager.append_csv(
                    "currencies.csv", currency_row, CURRENCY_FIELDNAMES
                )
    ...
    def list_currencies(self, active_only: bool = False) -> List[Currency]:
        ...
        if active_only:
            currencies = [
                c for c in currencies if c.get("is_active", "true").lower() == "true"
            ]
    ...
    def list_exchange_rates(
        ...
        for rate in rates:
            ...
            if (
                from_currency
                and rate.get("from_currency", "").upper() != from_currency.upper()
            ):
                continue
            if (
                to_currency
                and rate.get("to_currency", "").upper() != to_currency.upper()
            ):
                continue
    ...
    def update_exchange_rate(
        self, rate_id: str, rate_update: ExchangeRateUpdate
    ) -> ExchangeRate:
        """
    ...
    def delete_exchange_rate(self, rate_id: str) -> None:
        ...
        csv_manager.write_csv(
            "exchange_rates.csv", filtered_rates, EXCHANGE_RATE_FIELDNAMES
        )
