...
from models.analytics import (
    TrendAnalysis, TrendDataPoint, YoYReport, YoYComparison,
    SpendingPattern, CategoryInsight, CustomReport, DrillDownResult
)
...
class AnalyticsService:
    ...
    def get_trend_analysis(
        ...
        # Group by period
        period_data = self._group_by_period(filtered, period_type, metric, base_currency)
    ...
    def get_yoy_comparison(
        ...
        current_metrics = self._calculate_year_metrics(transactions, current_year, base_currency)
        previous_metrics = self._calculate_year_metrics(transactions, previous_year, base_currency)
        
        # Create comparisons
        income_comparison = self._create_yoy_comparison(
            "income",
            current_year, current_metrics['income'],
            previous_year, previous_metrics['income'],
            is_income=True
        )
        
        expense_comparison = self._create_yoy_comparison(
            "expenses",
            current_year, current_metrics['expenses'],
            previous_year, previous_metrics['expenses'],
            is_income=False
        )
        
        net_comparison = self._create_yoy_comparison(
            "net_income",
            current_year, current_metrics['net'],
            previous_year, previous_metrics['net'],
            is_income=True
        )
        
        # Calculate savings rates
        savings_rate_current = (current_metrics['net'] / current_metrics['income'] * 100) if current_metrics['income'] > 0 else 0
        savings_rate_previous = (previous_metrics['net'] / previous_metrics['income'] * 100) if previous_metrics['income'] > 0 else 0
    ...
    def get_category_insights(
        ...
        percentage_of_total = (total_spent / total_expenses * 100) if total_expenses > 0 else 0
    ...
    def _get_category_comparisons(
        ...
        for category_id, data in category_data.items():
            ...
            change_pct = (change / data['previous'] * 100) if data['previous'] > 0 else 0
    ...
    def _detect_category_pattern(
        ...
        if cv < 0.2:
            ...
            confidence = 0.9
            description = f"Consistent spending of ~{average_amount:.2f} per transaction"
        elif cv < 0.5:
            pattern_type = "moderate"
            confidence = 0.7
            description = f"Moderate variation in spending, average {average_amount:.2f}"
        else:
