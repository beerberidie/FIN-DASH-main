...
class Calculator:
    ...
    def calculate_category_spending(
        ...
        for tx in transactions:
            if tx.get('category_id') == category_id and tx.get('date', '').startswith(month):
                amount = float(tx.get('amount', 0))
                tx_currency = tx.get('currency', 'ZAR')
    ...
    def calculate_month_over_month_change(
        ...
        current = Calculator.calculate_monthly_totals(current_month, transactions, base_currency)
        previous = Calculator.calculate_monthly_totals(prev_month, transactions, base_currency)
