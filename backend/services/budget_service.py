...
class BudgetService:
    ...
    def calculate_budget_status(self, budget_id: str) -> Dict[str, Any]:
        ...
        status = {
            ...
            "total_planned": float(budget["needs_planned"])
            + float(budget["wants_planned"])
            + float(budget["savings_planned"]),
            "total_actual": sum(group_actual.values()),
        }

        # Calculate utilization percentages
        status["needs_utilization"] = (
            (status["needs_actual"] / status["needs_planned"] * 100)
            if status["needs_planned"] > 0
            else 0
        )
        status["wants_utilization"] = (
            (status["wants_actual"] / status["wants_planned"] * 100)
            if status["wants_planned"] > 0
            else 0
        )
        status["savings_utilization"] = (
            (status["savings_actual"] / status["savings_planned"] * 100)
            if status["savings_planned"] > 0
            else 0
        )
        status["total_utilization"] = (
            (status["total_actual"] / status["total_planned"] * 100)
    ...
    def get_current_month_budget(self) -> Dict[str, Any]:
        ...
        current_budget = next(
            (
                b
                for b in budgets
                if int(b["year"]) == now.year and int(b["month"]) == now.month
            ),
            None,
        )
