...
class CardService:
    ...
    def calculate_card_balance(self, card_id: str) -> Dict[str, float]:
        ...
        credit_limit = (
            float(card.get("credit_limit", 0)) if card.get("credit_limit") else 0.0
        )
        ...
        if card_type == "credit":
            ...
            )  # Add because current_balance is negative
        else:
    ...
    def get_card_analytics(self, card_id: str) -> Dict[str, Any]:
        ...
        # Calculate average
        expense_count = sum(
            1 for tx in card_transactions if float(tx.get("amount", 0)) < 0
        )
