...
class PDFExportService:
    ...
    def _create_header(
        self, config: ExportConfig, title: str, subtitle: Optional[str] = None
    ) -> List:
        """Create PDF header."""
    ...
    def export_transactions_pdf(
        ...
        subtitle = (
            f"Transaction Report - {date_range}" if date_range else "Transaction Report"
        )
        ...
        if filtered:
            ...
            transactions_table = Table(
                table_data,
                colWidths=[1 * inch, 2 * inch, 1.5 * inch, 1.5 * inch, 1.5 * inch],
            )
            ...
            elements.append(
                Paragraph(
                    "No transactions found for the selected criteria.", styles["Normal"]
                )
            )
    ...
    def export_financial_summary_pdf(self, config: ExportConfig) -> str:
        ...
        if goals:
            ...
            goals_table = Table(
                goals_data, colWidths=[2 * inch, 1.5 * inch, 1.5 * inch, 1 * inch]
            )
    ...
    def export_investment_portfolio_pdf(
        ...
        summary_data = [
            ...
            [
                "Total Value:",
                self._format_currency(portfolio_summary.total_value, config),
            ],
            [
                "Total Cost:",
                self._format_currency(portfolio_summary.total_cost, config),
            ],
            [
                "Profit/Loss:",
                self._format_currency(portfolio_summary.total_profit_loss, config),
            ],
    ...
    def export_debt_report_pdf(
        self, include_paid_off: bool, config: ExportConfig
    ) -> str:
        """
        ...
        elements.extend(
            self._create_header(
                config, "Debt Report", f"As of {datetime.now().strftime('%Y-%m-%d')}"
            )
        )
        ...
        if debts:
            ...
            table_data = [
                ["Name", "Type", "Balance", "Interest Rate", "Min Payment", "Due Date"]
            ]
                ...
                table_data.append(
                    [
                        ...
                        self._format_currency(
                            float(debt.get("current_balance", 0)), config
                        ),
                        f"{float(debt.get('interest_rate', 0)):.2f}%",
                        self._format_currency(
                            float(debt.get("minimum_payment", 0)), config
                        ),
                        debt.get("due_date", ""),
                    ]
                )
