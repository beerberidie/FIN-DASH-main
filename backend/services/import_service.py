...
class ImportService:
    ...
    def parse_csv(self, file_content: str, column_mapping: Optional[Dict[str, str]] = None,
                  bank_format: Optional[str] = None) -> Tuple[List[Dict], List[str]]:
        """
    ...
    def validate_transaction(self, row: Dict, column_mapping: Dict[str, str]) -> Tuple[bool, Optional[str]]:
        """
    ...
    def import_transactions(self, file_content: str, account_id: str,
        ...
        for idx, row in enumerate(rows):
            try:
                ...
                # Parse transaction
                transaction = self._parse_transaction(row, column_mapping, account_id, auto_categorize)
    ...
    def _parse_transaction(self, row: Dict, column_mapping: Dict[str, str],
                          account_id: str, auto_categorize: bool) -> Dict:
    ...
    def upload_and_parse_file(
        ...
        for txn in transactions:
            # Check for duplicates
            txn['is_duplicate'] = self._is_duplicate_transaction(txn, existing_transactions)
        ...
        import_id = f"import_{datetime.now().strftime('%Y%m%d%H%M%S')}_{uuid.uuid4().hex[:8]}"
    ...
    def confirm_import(
        ...
        if selected_transaction_indices is not None:
            transactions = [transactions[i] for i in selected_transaction_indices if i < len(transactions)]
    ...
    def _save_import_history(self, import_record: Dict[str, Any]) -> None:
        ...
        fieldnames = [
            'import_id', 'file_name', 'file_type', 'account_id',
            'total_transactions', 'imported_count', 'skipped_count',
