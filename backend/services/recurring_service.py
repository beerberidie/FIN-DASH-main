...
from models.recurring_transaction import RecurringTransaction, RecurringTransactionCreate, RecurringTransactionUpdate
...
class RecurringTransactionService:
    ...
    def _clean_row(self, row: dict) -> dict:
        ...
        for field in ['end_date', 'day_of_month', 'day_of_week', 'last_generated', 'next_due']:
        ...
        if 'day_of_month' in cleaned and cleaned['day_of_month'] is not None and isinstance(cleaned['day_of_month'], str):
            cleaned['day_of_month'] = int(cleaned['day_of_month'])

        if 'day_of_week' in cleaned and cleaned['day_of_week'] is not None and isinstance(cleaned['day_of_week'], str):
    ...
    def create(self, recurring_data: RecurringTransactionCreate) -> RecurringTransaction:
    ...
    def update(self, recurring_id: str, update_data: RecurringTransactionUpdate) -> Optional[RecurringTransaction]:
        ...
        if any(k in update_dict for k in ['frequency', 'start_date', 'day_of_month', 'day_of_week']):
    ...
    def get_due_transactions(self, check_date: Optional[date] = None) -> List[RecurringTransaction]:
