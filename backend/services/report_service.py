...
class ReportService:
    ...
    def generate_monthly_report(self, year: int, month: int) -> Dict:
        ...
        month_transactions = [
            ...
            if self._is_in_month(tx['date'], year, month)
        ]
        
        # Calculate income and expenses
        income = sum(float(tx['amount']) for tx in month_transactions if float(tx['amount']) > 0)
        expenses = abs(sum(float(tx['amount']) for tx in month_transactions if float(tx['amount']) < 0))
        net_income = income - expenses
        
        # Category breakdown
        category_spending = self._calculate_category_spending(month_transactions, categories)
    ...
    def generate_ytd_summary(self, year: int) -> Dict:
        ...
        ytd_transactions = [
            tx for tx in transactions
            if tx['date'].startswith(str(year))
        ]
        
        # Calculate totals
        income = sum(float(tx['amount']) for tx in ytd_transactions if float(tx['amount']) > 0)
        expenses = abs(sum(float(tx['amount']) for tx in ytd_transactions if float(tx['amount']) < 0))
        net_income = income - expenses
    ...
    def _calculate_category_spending(self, transactions: List[Dict], categories: List[Dict]) -> Dict:
    ...
    def _calculate_budget_performance(self, year: int, month: int, budgets: List[Dict]) -> Dict:
    ...
    def _calculate_mom_comparison(self, year: int, month: int, transactions: List[Dict]) -> Dict:
        ...
        current = [tx for tx in transactions if self._is_in_month(tx['date'], year, month)]
        current_income = sum(float(tx['amount']) for tx in current if float(tx['amount']) > 0)
        current_expenses = abs(sum(float(tx['amount']) for tx in current if float(tx['amount']) < 0))
        
        # Previous month transactions
        previous = [tx for tx in transactions if self._is_in_month(tx['date'], prev_year, prev_month)]
        prev_income = sum(float(tx['amount']) for tx in previous if float(tx['amount']) > 0)
        prev_expenses = abs(sum(float(tx['amount']) for tx in previous if float(tx['amount']) < 0))
    ...
    def _generate_insights(self, income: float, expenses: float, savings_rate: float,
                          budget_performance: Dict, category_spending: Dict) -> List[str]:
        ...
        if savings_rate >= 20:
            ...
            insights.append(f"Good job! You're saving {savings_rate:.1f}% of your income. Try to reach 20%.")
        else:
            insights.append(f"Your savings rate is {savings_rate:.1f}%. Consider reducing expenses to save more.")
        
        # Budget performance insight
        if budget_performance.get('has_budget'):
            total_util = budget_performance.get('total_utilization', 0)
            if total_util > 100:
                insights.append(f"You're {total_util - 100:.1f}% over budget this month. Review your spending.")
            elif total_util > 90:
                insights.append("You're close to your budget limit. Monitor spending carefully.")
            else:
                insights.append(f"You're at {total_util:.1f}% of your budget. Great control!")
        
        # Top spending category
        if category_spending:
            top_cat = max(category_spending.items(), key=lambda x: x[1]['amount'])
            insights.append(f"Your highest spending category is {top_cat[1]['name']} at R{top_cat[1]['amount']:.2f}.")
