...
class StatementParser:
    ...
    def parse_excel(file_path: Path) -> List[Dict[str, Any]]:
        ...
        for row in rows[1:]:
            ...
            # Extract description
            if (
                "description" in column_mapping
                and len(row) > column_mapping["description"]
            ):
            ...
            if "amount" in column_mapping and len(row) > column_mapping["amount"]:
                amount = StatementParser.parse_amount(
                    str(row[column_mapping["amount"]])
                )
            elif "debit" in column_mapping and "credit" in column_mapping:
                debit_val = (
                    row[column_mapping["debit"]]
                    if len(row) > column_mapping["debit"]
                    else None
                )
                credit_val = (
                    row[column_mapping["credit"]]
                    if len(row) > column_mapping["credit"]
                    else None
                )

                debit = (
                    StatementParser.parse_amount(str(debit_val)) if debit_val else None
                )
                credit = (
                    StatementParser.parse_amount(str(credit_val))
                    if credit_val
                    else None
                )
            ...
            if "balance" in column_mapping and len(row) > column_mapping["balance"]:
                balance = StatementParser.parse_amount(
                    str(row[column_mapping["balance"]])
                )
                if balance is not None:
                    transaction["balance"] = balance
    ...
    def parse_pdf(file_path: Path) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if match:
                date_str, description, debit_str, credit_str, balance_str = (
                    match.groups()
                )
                date = StatementParser.parse_date(date_str)
