...
class TrendAnalysis(BaseModel):
    ...
    trend_percentage: float = Field(..., description="Percentage change from first to last period")
...
class YoYReport(BaseModel):
    ...
    category_comparisons: List[Dict[str, Any]] = Field(default_factory=list, description="Category-level comparisons")
...
class SpendingPattern(BaseModel):
    ...
    pattern_type: str = Field(..., description="Pattern type (consistent, increasing, decreasing, seasonal)")
    average_amount: float = Field(..., description="Average spending amount")
    frequency: str = Field(..., description="Spending frequency (daily, weekly, monthly)")
...
class CategoryInsight(BaseModel):
    ...
    top_merchants: List[str] = Field(default_factory=list, description="Top merchants/descriptions")
    monthly_breakdown: List[TrendDataPoint] = Field(default_factory=list, description="Monthly spending breakdown")
...
class PredictionReport(BaseModel):
    ...
    metric: str = Field(..., description="Metric being predicted (income, expenses, net)")
    method: str = Field(..., description="Prediction method (moving_average, linear_regression, seasonal)")
...
class FinancialHealthScore(BaseModel):
    ...
    overall_status: str = Field(..., description="Overall status (excellent, good, fair, poor)")
    metrics: List[HealthMetric] = Field(..., description="Individual health metrics")
    strengths: List[str] = Field(default_factory=list, description="Financial strengths")
...
class HealthMetricBreakdown(BaseModel):
    ...
    emergency_fund_months: float = Field(..., description="Months of expenses in emergency fund")
    budget_adherence: float = Field(..., description="Budget adherence percentage")
    investment_diversification: float = Field(..., description="Investment diversification score")
    net_worth_trend: str = Field(..., description="Net worth trend (increasing, decreasing, stable)")
...
class CustomReportRequest(BaseModel):
    ...
    include_comparisons: bool = Field(default=False, description="Include period comparisons")
    group_by: str = Field(default="month", description="Grouping period (day, week, month, quarter)")
...
class CustomReport(BaseModel):
    ...
    savings_rate: float = Field(..., description="Savings rate percentage")
    category_breakdown: List[CategoryInsight] = Field(default_factory=list, description="Category insights")
    trends: List[TrendAnalysis] = Field(default_factory=list, description="Trend analyses")
    top_income_sources: List[Dict[str, Any]] = Field(default_factory=list, description="Top income sources")
...
class TrendRequest(BaseModel):
    ...
    metric: str = Field(..., description="Metric to analyze (income, expenses, net, category)")
    period_type: str = Field(default="monthly", description="Period type (monthly, quarterly, yearly)")
    ...
    category_id: Optional[str] = Field(None, description="Category ID for category-specific trends")
...
class HealthScoreRequest(BaseModel):
    ...
    include_investments: bool = Field(default=True, description="Include investment metrics")
    include_debts: bool = Field(default=True, description="Include debt metrics")
    reference_date: Optional[date_type] = Field(None, description="Reference date for calculation")
...
class DrillDownRequest(BaseModel):
    ...
    dimension: str = Field(..., description="Dimension to drill down (category, account, merchant, tag)")
    ...
    parent_id: Optional[str] = Field(None, description="Parent dimension ID for hierarchical drill-down")


class DrillDownResult(BaseModel):
    """Drill-down analysis result."""
    dimension: str = Field(..., description="Dimension analyzed")
    items: List[Dict[str, Any]] = Field(..., description="Drill-down items with metrics")
