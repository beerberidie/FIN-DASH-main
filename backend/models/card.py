...
class CardBase(BaseModel):
    ...
    name: str = Field(
        ...
        min_length=1,
        max_length=100,
        description="Card name (e.g., 'Visa Gold Card')",
    )
    card_type: Literal["credit", "debit", "prepaid", "virtual"] = Field(
        ..., description="Type of card"
    )
    last_four_digits: str = Field(
        ..., min_length=4, max_length=4, description="Last 4 digits of card number"
    )
    ...
    issuer: str = Field(
        ...
        min_length=1,
        max_length=100,
        description="Card issuer (e.g., 'Standard Bank')",
    )
    available_balance: float = Field(
        default=0.0, description="Available balance/credit"
    )
    current_balance: float = Field(
        default=0.0, description="Current balance (negative for credit cards)"
    )
    ...
    expiry_month: Optional[int] = Field(
        default=None, ge=1, le=12, description="Expiry month (1-12)"
    )
    expiry_year: Optional[int] = Field(
        default=None, ge=2000, le=2100, description="Expiry year"
    )
    is_active: bool = Field(default=True, description="Whether card is active")
    color: str = Field(
        default="#3b82f6", pattern=r"^#[0-9A-Fa-f]{6}$", description="Card color (hex)"
    )
...
class Card(CardBase):
    ...
    def from_csv(cls, row: dict) -> "Card":
        ...
        row_copy["credit_limit"] = (
            float(credit_limit) if credit_limit and credit_limit != "" else None
        )

        # Handle optional expiry fields
        expiry_month = row_copy.get("expiry_month", "")
        row_copy["expiry_month"] = (
            int(expiry_month) if expiry_month and expiry_month != "" else None
        )

        expiry_year = row_copy.get("expiry_year", "")
        row_copy["expiry_year"] = (
            int(expiry_year) if expiry_year and expiry_year != "" else None
        )
    ...
    def to_csv(self) -> dict:
        ...
        data["expiry_month"] = (
            str(data["expiry_month"]) if data["expiry_month"] is not None else ""
        )
        data["expiry_year"] = (
            str(data["expiry_year"]) if data["expiry_year"] is not None else ""
        )
