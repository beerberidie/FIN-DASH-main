...
class InvestmentPortfolioExportRequest(ExportRequest):
    ...
    investment_type: Optional[str] = Field(None, description="Filter by investment type")
...
class IncomeStatementExportRequest(ExportRequest):
    ...
    month: Optional[str] = Field(None, description="Month in YYYY-MM format (optional, defaults to current month)")


class BalanceSheetExportRequest(ExportRequest):
    """Balance sheet export request."""
    export_type: Literal["balance_sheet"] = "balance_sheet"
    as_of_date: Optional[date_type] = Field(None, description="Balance sheet as of date (defaults to today)")
...
class ExportConfig(BaseModel):
    ...
    company_name: str = Field(default="FIN-DASH", description="Company/App name for headers")
    ...
    number_format: str = Field(default=",.2f", description="Number format for currency values")
    page_size: Literal["A4", "Letter"] = Field(default="A4", description="PDF page size")
    orientation: Literal["portrait", "landscape"] = Field(default="portrait", description="PDF orientation")
