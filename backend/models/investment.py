...
INVESTMENT_FIELDNAMES = [
    'id', 'symbol', 'name', 'type', 'currency', 'quantity', 'average_cost',
    'current_price', 'last_updated', 'notes', 'created_at', 'updated_at'
]

INVESTMENT_TRANSACTION_FIELDNAMES = [
    'id', 'investment_id', 'date', 'type', 'quantity', 'price', 'fees',
...
class InvestmentBase(BaseModel):
    ...
    symbol: str = Field(..., min_length=1, max_length=20, description="Stock/crypto symbol (e.g., AAPL, BTC)")
    ...
    current_price: float = Field(default=0.0, ge=0, description="Current market price per unit")
...
class InvestmentTransactionBase(BaseModel):
    ...
    total_amount: float = Field(..., description="Total amount (quantity * price + fees for buy, quantity * price - fees for sell)")
...
class PriceUpdate(BaseModel):
    ...
    last_updated: Optional[str] = Field(None, description="Update date (defaults to today)")
