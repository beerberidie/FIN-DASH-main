...
class RecurringTransaction(BaseModel):
    ...
    # Recurrence settings
    frequency: Literal["daily", "weekly", "biweekly", "monthly", "quarterly", "yearly"] = Field(
        ..., description="Frequency of recurrence"
    )
    start_date: str = Field(..., description="Date to start generating transactions (YYYY-MM-DD)")
    ...
    # Status and metadata
    is_active: bool = Field(True, description="Whether this recurring transaction is active")
    last_generated: Optional[str] = Field(None, description="Date of last generated transaction")
    next_due: Optional[str] = Field(None, description="Next due date for transaction generation")
