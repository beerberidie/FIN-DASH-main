...
def create_accounts(dry_run: bool = False, verbose: bool = False) -> Dict[str, str]:
    ...
    for account in accounts:
        if dry_run:
            print(f"  [DRY RUN] Would create account: {account['name']} ({account['type']}) - R{account['opening_balance']:.2f}")
            account_map[account['name']] = f"acc_{account['name'].lower().replace(' ', '_')}"
        else:
            try:
                ...
                if response.status_code == 201:
                    ...
                    if verbose:
                        print_success(f"  Created account: {account['name']} (ID: {data['id']})")
                    else:
                    ...
                    print_error(f"  Failed to create account: {account['name']} - {response.text}")
...
def create_categories(dry_run: bool = False, verbose: bool = False) -> Dict[str, str]:
    ...
    categories = [
        # Expense categories
        {"name": "Food", "group": "needs", "color": "#ef4444", "icon": "UtensilsCrossed"},
        {"name": "Gifts", "group": "wants", "color": "#ec4899", "icon": "Gift"},
        {"name": "Health/Medical", "group": "needs", "color": "#10b981", "icon": "Heart"},
        ...
        {"name": "Other", "group": "wants", "color": "#64748b", "icon": "MoreHorizontal"},
        {"name": "Wifi", "group": "needs", "color": "#3b82f6", "icon": "Wifi"},
        {"name": "Gap Cover", "group": "needs", "color": "#059669", "icon": "Shield"},
        {"name": "YouTube Premium", "group": "wants", "color": "#dc2626", "icon": "Youtube"},
        {"name": "YouTube Music", "group": "wants", "color": "#f97316", "icon": "Music"},
        {"name": "Spotify", "group": "wants", "color": "#22c55e", "icon": "Music"},
        {"name": "Netflix", "group": "wants", "color": "#ef4444", "icon": "Tv"},
        {"name": "Bike Insurance", "group": "needs", "color": "#f59e0b", "icon": "Bike"},
        {"name": "Bank Charges", "group": "needs", "color": "#6366f1", "icon": "Building"},
        {"name": "Cell Phone", "group": "needs", "color": "#8b5cf6", "icon": "Smartphone"},
        {"name": "Liberty (Insurance)", "group": "needs", "color": "#0ea5e9", "icon": "Shield"},
        
        # Income categories
        {"name": "Paycheck", "group": "income", "color": "#10b981", "icon": "Wallet"},
        {"name": "Savings", "group": "savings", "color": "#3b82f6", "icon": "PiggyBank"},
        {"name": "Bonus", "group": "income", "color": "#22c55e", "icon": "TrendingUp"},
        {"name": "Interest", "group": "income", "color": "#06b6d4", "icon": "Percent"},
        {"name": "Other Income", "group": "income", "color": "#84cc16", "icon": "DollarSign"},
        {"name": "Custom Category", "group": "income", "color": "#64748b", "icon": "Circle"},
    ]
    ...
    for category in categories:
        if dry_run:
            print(f"  [DRY RUN] Would create category: {category['name']} ({category['group']})")
            category_map[category['name']] = f"cat_{category['name'].lower().replace(' ', '_').replace('/', '_')}"
        else:
            try:
                ...
                if response.status_code == 201:
                    ...
                    if verbose:
                        print_success(f"  Created category: {category['name']} (ID: {data['id']})")
                    else:
                    ...
                    print_error(f"  Failed to create category: {category['name']} - {response.text}")
            except Exception as e:
                print_error(f"  Error creating category {category['name']}: {str(e)}")
    
    return category_map

def create_credit_card(account_map: Dict[str, str], dry_run: bool = False, verbose: bool = False) -> bool:
...
def create_budget(dry_run: bool = False, verbose: bool = False) -> bool:
    ...
    # Calculate totals
    needs_total = 2000.00 + 2017.00 + 2000.00 + 2000.00 + 500.00 + 300.00 + 138.00 + 338.00 + 150.00 + 250.00 + 700.00  # 10,393
    wants_total = 0.00 + 3000.00 + 0.00 + 0.00 + 100.00 + 60.00 + 90.00 + 159.00  # 3,409
    savings_total = 0.00
    ...
    if dry_run:
        ...
        print(f"    Total: R{budget['needs_planned'] + budget['wants_planned'] + budget['savings_planned']:.2f}")
        return True
    else:
        try:
            ...
            if response.status_code == 201:
                if verbose:
                    ...
                    print_success(f"  Created budget: R{budget['needs_planned'] + budget['wants_planned']:.2f} total")
                return True
            elif response.status_code == 400 and "already exists" in response.text:
